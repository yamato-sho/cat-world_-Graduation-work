...
function updateMinimap() {
    // tiles変数はDOMContentLoadedスコープ内にあるため、
    // ここで再度取得するか、tilesをグローバルに移動する必要があります。
    // tilesをグローバルに移動するのが最も簡単です。
    const tiles = document.querySelectorAll('.tile',);
    
    const minimapContent = document.getElementById('minimapContent');
    if (!minimapContent) return; // エラー回避
    ...
    // 実際のスクロール可能領域のサイズを取得
    // NOTE: document.body.scrollHeight/Widthは、全てのコンテンツが読み込まれて初めて正確な値になります。
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const canvas = document.getElementById('canvas');
    // HTMLからすべてのタイル要素を取得 (グローバル関数 updateMinimap でも使用するため、定数ではなくグローバル変数にするか、都度取得する必要あり)
    const tiles = document.querySelectorAll('.tile',);
    ...
    // --- ★ 修正箇所 3: モーダルを背景クリックで閉じる処理を汎用的に修正 ★ ---

    // スクロールでミニマップを更新
    // updateMinimapがグローバルスコープにあるため、ここで呼び出し可能
...
window.addEventListener('load', function() {
    ...
    // 2. ミニマップを確実に初期描画
    // document.body.scrollWidth/Heightが確定するため、ここで呼び出すのが最適
    updateMinimap(); 
});
});
